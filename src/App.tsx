...
const TermTooltip = ({ title, content }: { title: string; content: string | React.ReactNode }) => {
  ...
  return (
    <div className="relative inline-flex items-center ml-1.5 group">
      ...
      >
        <HelpCircle size={12} className={cn("transition-colors", isVisible ? "text-gold" : "text-jade/40")} />
      </button>
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="absolute z-[100] bottom-full left-1/2 -translate-x-1/2 mb-2 w-60 p-3 bg-black/90 backdrop-blur-md border border-white/10 rounded-lg shadow-2xl pointer-events-none"
          >
            <div className="text-[11px] text-zinc-200 leading-relaxed">
              {typeof content === 'string' ? (
                <p>
                  <span className="font-bold text-gold mr-1">{title}：</span>
                  {content}
                </p>
              ) : content}
            </div>
              ...
              <div className="grid lg:grid-cols-3 gap-12">
                <div className="lg:col-span-1 space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { label: "干支纪年", value: calcData.wylq_summary.ganzhi, color: "text-jade", tooltip: "东方古老的纪年方式，由天干与地支组合，反映年份基础宇宙磁场能量。" },
                      { label: "岁运", value: calcData.wylq_summary.suiyun, color: "text-gold", tooltip: "全年的气象总司令。主导全年五行能量的偏向（如木、火、土、金、水）。" },
                      { label: "司天", value: calcData.wylq_summary.sitian, color: "text-orange-400", tooltip: "主导上半年（大寒至小暑）的气候基调，是决定全年“天气”属性的核心。" },
                      { label: "在泉", value: calcData.wylq_summary.zaiquan, color: "text-purple-400", tooltip: "主导下半年（大暑至小寒）的气候基调，是决定全年“地气”属性的核心。" }
                    ].map((item, i) => (
                        ...
                        <TermTooltip 
                          ...
                          content={
                            <div className="space-y-2">
                              <p><span className="text-gold font-bold">太过：</span>该年份对应的五行能量过盛，容易产生因能量亢奋导致的身体压力。</p>
                              <p><span className="text-gold font-bold">不及：</span>该年份对应的五行能量虚弱，代表该维度的身体防线较为脆弱，需针对性调养。</p>
                            </div>
                          } 
                        />
                      </div>
                    ...
                    <div className="glass-panel p-6 border-l-2 border-l-gold">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <TermTooltip 
                          ...
                          content={
                            <div className="space-y-2">
                              <p><span className="text-gold font-bold">主位/主：</span>每年固定不变的五行节律（如春木夏火），是生命适应自然的常规背景。</p>
                              <p><span className="text-gold font-bold">客位/客：</span>逐年轮动、变幻无常的动态气象能量，是导致体质波动和健康风险的主要诱因。</p>
                            </div>
                          } 
                        />
                      </div>
          ...
          >
            ...
            {!user.loggedIn ? (
              ...
              <div className="space-y-8">
                ...
                {/* Referral System */}
                <div className="glass-panel p-8 bg-gradient-to-br from-gold/5 to-transparent border-gold/10 space-y-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gold/10 flex items-center justify-center border border-gold/20">
                      ...
                      <div className="space-y-0.5">
                        <h3 className="text-lg font-bold text-white serif">推荐奖励计划</h3>
                        <p className="text-zinc-500 text-xs">邀请好友加入，双方各得 2 颗草药</p>
                      </div>
                    </div>
                  </div>
                  
                  <div className="flex flex-col md:flex-row gap-4 items-center">
                    <div className="flex-1 w-full bg-white/[0.03] border border-white/10 rounded-2xl px-6 py-4 flex items-center justify-between group hover:border-gold/30 transition-all">
                      <div className="space-y-1">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-widest">您的专属推荐码</span>
                        <div className="text-2xl font-black text-gold tracking-widest serif">
                          {user.user?.invite_code || '加载中...'}
                        </div>
                      </div>
                      <button 
                        onClick={() => {
                          if (user.user?.invite_code) {
                            navigator.clipboard.writeText(user.user.invite_code);
                            alert('推荐码已复制');
                          }
                        }}
                    ...
                    <button 
                      ...
                      className="w-full md:w-auto bg-gold hover:bg-gold-dark text-black font-bold px-8 py-5 rounded-2xl transition-all shadow-lg shadow-gold/20 flex items-center justify-center gap-2 whitespace-nowrap"
                    >
