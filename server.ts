...
3. **必须使用标准的 Markdown 标题格式**（## 标题）。
4. **强制换行规范**：在每一个标题（## 或 ###）之后、以及每一个列表项之间，必须使用双换行符（\n\n），确保输出文本的物理间隔与呼吸感。
5. 使用列表（- 或 1.）来列举风险点和建议。
6. 关键结论使用引用块（>）强调。
7. **严禁在 Markdown 符号前添加反斜杠（如严禁输出 \## 或 \-）**。
8. **严禁使用 -> 或 => 等符号**。
9. **严禁在输出中使用任何反斜杠 \ 进行转义**。直接输出干净的 Markdown 文本。
10. **严禁在正文中出现散乱的 # 符号**。
